...
import { biomeAt, elevationAt, groundHeightAt, WORLD } from './WorldGen.ts';
...
export interface ExteriorBuild {
  ...
  /** Streams terrain/props around the player and advances wind + fires. */
  update(elapsed: number, dt: number, playerPos: Vector3): void;
  ...
  stats(): { chunks: number; pending: number; biome: string };
}
...
export function buildExterior(assets: AssetManager, settings: QualitySettings): ExteriorBuild {
  ...
  const scatter: ScatterStreamer = createScatter(registry, wind);
  group.add(scatter.group);

  const landmarks: LandmarkStreamer = createLandmarks(registry);
  ...
  const portal: PortalBuild = (() => {
    ...
    registry.add('spawn', { x: -5.4, z: 0, r: 2.9, top: plazaY + slabH, solid: true });
    registry.add('spawn', { x: 5.4, z: 0, r: 2.9, top: plazaY + slabH, solid: true });
  ...
  const collide = (p: Vector3): void => {
    ...
    registry.forEachNear(p.x, p.z, (o) => {
      if (!o.solid) return;
      if (p.y >= o.top - 0.15) return; // standing on top of it
      const dx = p.x - o.x;
      const dz = p.z - o.z;
      const minD = o.r + pr;
    ...
    for (let i = 0; i < 6; i++) {
      if (elevationAt(p.x, p.z) >= shoreY) break;
      const gx = elevationAt(p.x + 2, p.z) - elevationAt(p.x - 2, p.z);
      const gz = elevationAt(p.x, p.z + 2) - elevationAt(p.x, p.z - 2);
  ...
  const blocksCamera = (x: number, y: number, z: number): boolean => {
    if (y < groundHeightAt(x, z) + 0.3) return true;
    let hit = false;
    registry.forEachNear(x, z, (o) => {
      if (hit || y > o.top) return;
      const dx = x - o.x;
      const dz = z - o.z;
      const r = o.r * 0.72;
  ...
  const update = (elapsed: number, dt: number, playerPos: Vector3): void => {
    ...
    // Streaming is budgeted per frame so walking never causes a hitch.
    terrain.pump();
    scatter.pump();
    landmarks.pump();
  ...
  return {
    ...
    stats: () => ({
      ...
      biome: biomeAt(centre.x, centre.z),
    }),
  };
}

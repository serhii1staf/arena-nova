...
export function buildExterior(assets: AssetManager, settings: QualitySettings): ExteriorBuild {
  ...
  const floorHeightAt = (x: number, z: number, fromY?: number): number => {
    ...
    // who is asking, so an unbounded pass here can lift you onto anything registered overhead —
  ...
  const update = (
    ...
    villagers.update(dt, playerPos, air.nightFactor, villages, collide, floorHeightAt);
    livestock.update(dt, playerPos, villages, collide, floorHeightAt);

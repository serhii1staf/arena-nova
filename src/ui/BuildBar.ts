...
export class BuildBar {
  ...
  update(): void {
    if (!this.root) return;
    // The door prompt and the E key moved out to `Interact`: there is one prompt under
    // the crosshair and one thing E does, and the scene is the only place that knows
    // whether that is a door, a stick on the ground or a workbench.
